...
sourcesRouter.get("/:sourceId", async (req: Req, res: Res) => {
  try {
    const owner = getOwnerContext(req);
    const item = await getSourceById(owner, req.params.sourceId);
    if (!item) {
...
sourcesRouter.post("/", async (req: Req, res: Res) => {
  try {
    ...
    const owner = getOwnerContext(req);
    const item = await createSource(owner, req.body.document);
    sendItem(res, item as unknown as Record<string, unknown>);
  } catch (error) {
    handleRouteError(res, error);
  }
...
sourcesRouter.patch("/:sourceId", async (req: Req, res: Res) => {
  try {
...
sourcesRouter.delete("/:sourceId", async (req: Req, res: Res) => {
  try {
    const owner = getOwnerContext(req);
    const item = await softDeleteSource(owner, req.params.sourceId);
    sendItem(res, item as unknown as Record<string, unknown>);
  } catch (error) {
    handleRouteError(res, error);
  }
