/**
 * Rules for which persisted lesson steps must have a board FEN and localized
 * correct/incorrect feedback before runtime playback export (GET …/playback).
 *
 * Keep in sync with `client/src/lesson-system/utils/runtimeExportPolicy.ts`.
 */
...
/**
 * True when every authoring moment is text-only (intro / wrap-up), so an empty
 * initial FEN is acceptable for Roblox-style playback that only shows copy.
 */
...
/** True when the learner can answer incorrectly; feedback strings should be required. */
