/**
 * Duplicated from client/src/engine/scan/createScanApi.ts for server-side import
 * (client package is ESM-only; ts-node cannot require those modules).
 * Keep in sync when the Scan WASM bridge changes.
 */
type ScanLike = {
  cwrap?: (...args: any[]) => any;
  __lastStdout?: string[];
...
declare global {
  ...
  var __scanProgressCallback: ((raw: string) => void) | undefined;
}
...
export function createScanApi(scan: ScanLike, options?: { silent?: boolean }) {
  ...
  return {
    async analyze(payload: AnalyzePayload): Promise<AnalyzeResult> {
      ...
      try {
        ...
        const suffix = extraLogs.length > 0 ? ` | logs: ${extraLogs.join(" | ")}` : "";
