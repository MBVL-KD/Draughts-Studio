...
import type { AskSelectPiecesInteraction } from "../types/authoring/interactionTypes";
...
export default function AuthoringAskSelectPiecesMomentFields({
  moment,
  language,
  onApply,
  studioSelection,
  ...
  const patchIx = (partial: Partial<AskSelectPiecesInteraction>) => {
    onApply({
      ...moment,
  ...
  return (
    <div style={rootStyle}>
      <div style={titleStyle}>{t("askSelectPieces (preview)", "askSelectPieces (preview)")}</div>
      <p style={hintStyle}>
        {t(
          "MVP: targets are square ids; use occupied squares on the board.",
          "MVP: doelen zijn veld-nummers; gebruik bezette velden op het bord."
        )}
      </p>
